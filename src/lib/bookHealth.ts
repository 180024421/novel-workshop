...
export type HealthDimension = {
  id: "beats" | "hooks" | "craft" | "summaries";
  label: string;
  score: number;
  max: number;
  detail: string;
...
export function scoreFromProgress(opts: {
  ...
  const beatsScore = Math.round(beatsRatio * 25);

  const openHooks = Math.max(0, opts.openHooks);
  // 开放钩子适量：0–8 满分，过多扣分
  let hooksScore = 25;
  if (openHooks === 0 && chaptersWithBody > 2) hooksScore = 18;
  else if (openHooks > 20) hooksScore = 8;
  else if (openHooks > 12) hooksScore = 15;
  else if (openHooks > 8) hooksScore = 20;
  ...
  const density =
    opts.craftSampleWords > 0
      ...
      : 0;
  // 每千字工艺病：0→25，≥8→0
  const craftScore = Math.round(clamp(25 - density * (25 / 8), 0, 25));
  ...
  const summariesScore = Math.round(summaryRatio * 25);
...
export async function projectHasCraftRules(
  ...
  try {
    ...
    const blob = `${style}\n${taboo}`;
    return /写作工艺|啰嗦|电报文|顶真|标语|改写命运/.test(blob);
  } catch {
    return false;
  }
}
...
export async function revisionDiskStats(
  ...
  const items = await listBackups(root, join);
  return {
    count: items.length,
    approxWords: items.reduce((s, i) => s + (i.words || 0), 0),
  };
}
